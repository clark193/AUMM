...
export function MemberTransparencyFeed() {
  ...
  useEffect(() => onSnapshot(query(collection(getFirebaseServices().db, "transparency"), where("status", "==", "published")), (snapshot) => setRows(snapshot.docs.map((item) => ({ id: item.id, ...item.data() } as TransparencyRow))), (reason) => setError(firebaseErrorMessage(reason, "Não foi possível carregar a transparência."))), []);
