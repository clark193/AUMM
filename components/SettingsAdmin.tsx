...
const initial = { associationName: "Associação União Maior Motoboys", acronym: "AUMM", cnpj: "45.115.209/0001-39", email: "contato@aumm.com.br", phone: "", whatsapp: "", address: "", city: "", state: "", instagram: "https://www.instagram.com/aumm.oficial/", facebook: "", siteMessage: "", cardValidityText: "Válida enquanto o associado estiver ativo" };
...
export function SettingsAdmin() {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      await writeAdminAudit({ action: "SETTINGS_UPDATED", resource: "settings", resourceId: "public", description: "Atualizou os dados institucionais, contato ou validade da carteirinha." }).catch(() => undefined);
  ...
  return <section className="panel operational-editor"><div className="panel-head"><div><h3><Settings size={18} /> Dados institucionais e página de contato</h3><p>Estas informações aparecem no rodapé, na página “Contato” e na carteirinha.</p></div></div><form onSubmit={submit}><div className="form-grid">
    ...
    <label className="field"><span>Sigla</span><input required value={form.acronym} onChange={(event) => setForm({ ...form, acronym: event.target.value })} /></label><label className="field"><span>CNPJ</span><input value={form.cnpj} onChange={(event) => setForm({ ...form, cnpj: event.target.value })} /></label><label className="field"><span>E-mail</span><input type="email" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} /></label>
