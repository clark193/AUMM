...
export function MemberCardContent() {
  ...
  const verifyUrl =
    typeof window !== "undefined" && member?.memberNumber
      ? `${window.location.origin}${withBasePath("/verificar")}?token=${encodeURIComponent(member.memberNumber)}`
      : "";
  async function share() {
    if (!verifyUrl) return;
    if (navigator.share)
      await navigator.share({
        title: "Carteirinha AUMM",
        text: "Verificação da minha carteirinha AUMM",
        url: verifyUrl,
      });
      ...
      <div className="id-page-inner">
        <div className="id-actions">
          ...
          <span style={{ display: "flex", gap: 8 }}>
            ...
            <button onClick={share} disabled={!verifyUrl}>
        ...
        <div className="id-card">
          <div className="id-top">
            <div className="id-brand">
              <Image src={withBasePath("/logo.png")} width={68} height={68} alt="AUMM" />
              <span>
                <strong>AUMM</strong>
                <small>Carteira de associado</small>
              </span>
            </div>
            <small>BLUMENAU · SC</small>
          </div>
          <div className="id-body">
            ...
            <div className="id-data">
              ...
              <h1>{member?.fullName || "Carregando..."}</h1>
              <strong>{member?.role || "ASSOCIADO"}</strong>
              <p>
                {member?.memberNumber || "Número em processamento"} ·{" "}
                {member?.status === "active" ? "ATIVO" : "EM ANÁLISE"}
              </p>
              <p>
                Desde{" "}
            ...
            {verifyUrl && (
              <div className="qr-box">
                <QRCodeSVG value={verifyUrl} size={94} level="H" />
              </div>
            )}
          </div>
        </div>
        ...
        >
          <Download size={17} /> Baixar como PDF
        </button>
      </div>
    </main>
  );
