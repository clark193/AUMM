...
export default function HomePage() {
  return <PublicShell>
    ...
    <section className="section"><div className="container">
      ...
      {news.length ? <div className="news-grid">{news.map((item,i)=><Link className={i===0?"news-card news-featured":"news-card"} href={`/noticias/${item.slug}`} key={item.slug}><div className="news-art"><span>{item.category}</span>{i===0?<ShieldCheck/>:i===1?<Handshake/>:<CalendarDays/>}</div><div className="news-body"><small>{item.date}</small><h3>{item.title}</h3><p>{item.summary}</p><span className="read-more">Ler notícia <ArrowRight size={16}/></span></div></Link>)}</div> : <div className="empty-state">Nenhuma notícia publicada.</div>}
    </div></section>
    <BoardWall/>
  </PublicShell>;
}
