...
export const operationalModules: Record<string, AdminModuleConfig> = {
  ...
  transparencia: {
    ...
    singular: "publicação",
    titleField: "title",
    searchFields: ["title", "category", "competence"],
    fields: [
      { key: "title", label: "Título", required: true, full: true },
      { key: "category", label: "Categoria", required: true },
      { key: "competence", label: "Competência", placeholder: "Agosto/2026" },
      { key: "description", label: "Descrição", type: "textarea", required: true, full: true },
      { key: "url", label: "Link do documento", type: "url", placeholder: "https://...", full: true },
      { key: "status", label: "Status", type: "select", required: true, options: statusOptions },
    ],
  },
  financeiro: {
